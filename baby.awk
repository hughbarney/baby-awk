...
# this version supports does not support ADD and LDA negates on loading.
#
...
BEGIN {
	...
	tempfile = ".temp"
	n = split("CONST GET OUT LDA STA SUB CMP JMP JNZ STP", x)
...
#		else if (code == op["ADD"])  { db("ADD",addr,acc); acc += mem[addr] }
		...
		else if (code == op["JMP"])   { db("JMP",addr,acc); pc = addr }
		else if (code == op["JNZ"])   { db("JNZ",addr,acc); if (acc != 0) pc = addr }
		else if (code == op["CMP"])   { db("CMP",addr,acc); if (acc < 0) pc = pc + 1 }
